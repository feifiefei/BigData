...
public class MockDataApp {
    ...
    /** ClickHouse **/
    private static List<String> ckTableNames = Arrays.asList("tbl_address","tbl_areas","tbl_codes","tbl_collect_package","tbl_consumer_address_map","tbl_consumer_sender_info","tbl_courier","tbl_customer","tbl_deliver_package","tbl_dot","tbl_dot_transport_tool","tbl_express_bill","tbl_express_package","tbl_out_warehouse","tbl_pkg","tbl_push_warehouse","tbl_route","tbl_transport_record","tbl_transport_tool","tbl_warehouse","tbl_warehouse_emp","tbl_warehouse_receipt","tbl_warehouse_transport_tool","tbl_warehouse_vehicle_map","tbl_waybill");
    private static String DROP_CK_TABLE_SQL = "DROP TABLE IF EXISTS ?";
    private static String CLEAN_CK_TABLE_SQL = "TRUNCATE TABLE IF EXISTS ?";
    /** PUBLIC SQL **/
    ...
    private static final DBHelper mysqlHelper = DBHelper.builder()
            ...
            .build();
    // Kudu
    private static final KuduHelper kuduHelper = null ;
/*
...
*/
    // ClickHouse JDBC
    private static final DBHelper ckHelper = null ;
/*
    private static final DBHelper ckHelper = DBHelper.builder()
            .withDialect(DBHelper.Dialect.ClickHouse)
            .withDriver("ru.yandex.clickhouse.ClickHouseDriver")
            .withUrl("jdbc:clickhouse://node2.itcast.cn:8123/logistics")
            .withUser("root")
            .withPassword("123456")
            .build();
*/
    ...
    public static void main( String[] args) {
        ...
        /** ==== 初始化ClickHouse ==== **/
        //Connection connection3 = ckHelper.getConnection();
        // 清空表
        if(isClean) {
            // 清空Oracle表
...
/*
            ...
            ckTableNames.forEach(tableName -> {
                try {
                    System.out.println("==== 开始删除ClickHouse表："+tableName+" ====");
                    RDBTool.update(CLEAN_CK_TABLE_SQL, tableName, (sql, table) -> executeUpdate(connection3, sql.replaceAll("\\?", table), 2));
                    System.out.println("==== 完成删除ClickHouse表："+tableName+" ====");
                    Thread.sleep(200*2);
                } catch (Exception e) {}
            });
*/
    ...
    private static Map<String, List<String>> buildOracleTableDML() {
        ...
        oracleTableNames.forEach(table -> {
            ...
            try {
                ...
                tableDatPath = MockDataApp.class.getResource("/" + table + DAT_SUFFIX).getPath();
            } catch (Exception e) { }
    ...
    private static Map<String, LinkedHashMap<String,String>> getOracleAllTableSchema(String path) {
        if(StringUtils.isEmpty(path)) {
            path = MockDataApp.class.getResource(ORACLE_DDL_SQL_FILE).getPath();
        }
    ...
    private static Map<String, List<String>> buildMySQLTableDML() {
        ...
        mysqlTableNames.forEach(table->{
            ...
            try {
                // 根据表名获取类路径下的"表名.csv"绝对路径
                tableDatPath = MockDataApp.class.getResource("/" + table.replace("crm","tbl") + DAT_SUFFIX).getPath();
            } catch (Exception e) {
            }
            if(!StringUtils.isEmpty(tableDatPath)) {
                StringBuilder insertSQL = new StringBuilder();
    ...
    private static Map<String, LinkedHashMap<String,String>> getMySQLAllTableSchema(String path) {
        if(StringUtils.isEmpty(path)) {
            path = MockDataApp.class.getResource(MYSQL_DDL_SQL_FILE).getPath();
        }
        Map<String, LinkedHashMap<String,String>> tableSchema = new LinkedHashMap<>();
        try {
            List<String> ddlSQL = FileUtils.readLines(new File(path), ENCODING);
    ...
    private static void executeUpdate(Connection connection, String sql, int dbType) {
        ...
        try {
            if (null==connection||connection.isClosed()) {
                ...
                if(dbType==2){
                    connection = ckHelper.getConnection();
                }
            }
